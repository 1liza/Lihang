...
- 本章最后的三个部分，这三个问题可以对比着看。

- 关于损失函数，风险函数与目标函数注意体会差异

1.损失函数（Loss Function ）是定义在单个样本上的，算的是一个样本的误差。

1.代价函数（Cost Function ）是定义在整个训练集上的，是所有样本误差的平均，也就是损失函数的平均。代价函数也被称为平方误差函数，有时也被称为平方误差代价函数，之所以要出误差的平方和，是因为误差平方代价函数对于大多数问题，特别是回归问题，都是一个合理的选择。

1.风险函数：

1.目标函数（Object Function）定义为：最终需要优化的函数。等于经验风险+结构风险（也就是Cost Function + 正则化项）。

**不仅要让经验风险尽量小，还要让结构风险尽量小**定义了一个函数J(f) ，这个函数专门用来度量模型的复杂度，在机器学习中也叫正则化(regularization)。常用的有L_1 ,L_2 范数。我们最终的优化函数是：min\frac{1}{N}\sum_{i=1}^{N}L(y_{i},f(x_{i}))+\lambda J(f) ，即最优化经验风险和结构风险，而这个函数就被称为目标函数。
